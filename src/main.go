...
func main() {
	...
	header, token, err := i3bar.ParseHeader(stdInDec)
	if err != nil {
		fmt.Printf("Error during header parsing %s", err.Error())
		os.Exit(1)
	}
	err = stdOutEnc.Encode(header)
	if err != nil {
		fmt.Printf("Error during header encoding %s", err.Error())
		os.Exit(1)
	}
	fmt.Println(token)
	for stdInDec.More() {
		blocks, err := i3bar.ParseBlocks(stdInDec)
		if err != nil {
			fmt.Printf("Error during block %v", err)
			panic("Error during block parsing")
		}
		lastBlock := blocks[len(blocks)-1]
		// fmt.Printf("last block %v \n", lastBlock)
		i3bar.AppendBlock(stdOutEnc, blocks, lastBlock)
		fmt.Print(",")
	}
}
